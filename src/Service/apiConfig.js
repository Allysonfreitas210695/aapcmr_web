...
const handleErrorResponse = (error) => {
    if (error.response) {
        if (error.response.status === 401 || error.response.status === 403) {
            return { status: error.response.status, message: "Sua sessão expirou. Realize o acesso novamente" };
        } else {
            try {
                return { status: error.response.data.status, message: error.response.data ? error.response.data.message : error.response.message };
            } catch {
...
const api_POST = async (endpoint, obj, contentType) => makeRequest('POST', endpoint, obj, contentType);
...
const api_GET_Unauthorize = async (endpoint) => makeRequest('GET', endpoint, null, 'application/json', false);

const api_POST_Unauthorize = async (endpoint, obj) => makeRequest('POST', endpoint, obj, 'application/json', false);
