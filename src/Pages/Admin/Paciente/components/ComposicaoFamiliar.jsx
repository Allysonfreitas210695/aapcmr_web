...
export default function ComposicaoFamiliar({ paciente, loadPaciente, showLoading, setPaciente }) {
  ...
  const [columns] = useState([
    ...
    {
      title: 'Idade do Familiar',
      field: 'idadeFamiliar',
      render: (rowDate) => <>{rowDate.idadeFamiliar} Ano(s)</>
    },
  ...
  const handleRemoveTramentoPaciente = async ({ id }) => {
    const resposta = await ShowConfirmation({
      title: '',
      text: 'Você tem certeza que quer deletar o familiar do paciente?'
    });
    if (resposta) {
      try {
        showLoading(true);
  ...
  const handleEditTramentoPaciente = (composicaoFamiliar) => {
    setVinculosFamiliar({
      value: composicaoFamiliar.vinculoFamiliar,
      label: composicaoFamiliar.vinculoFamiliar
    });
      ...
      <Row>
        <Col sm={12} lg={6}>
          <ControlledInput
            ...
            label={
              <>
                <span className="text-danger">*</span> Nome do Familiar
              </>
            }
            type="text"
            rules={{
        ...
        <Col sm={12} lg={3}>
          <ControlledInput
            ...
            label={
              <>
                <span className="text-danger">*</span> Idade do Familiar
              </>
            }
            type="number"
            rules={{
        ...
        <Col sm={12} lg={3} style={{ zIndex: 9999 }}>
          <SelectCustom
            ...
            label={
              <>
                <span className="text-danger">*</span> Vínculo Familiar
              </>
            }
        ...
        <Col sm={12} lg={2}>
          <ControlledInput
            ...
            label={
              <>
                <span className="text-danger">*</span> Renda do Familiar
              </>
            }
            type="number"
            maxlenght={15}
