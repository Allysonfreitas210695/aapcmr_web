...
export default function RelatorioMovimentacao() {
    ...
    const exportToExcel = async () => {
        ...
        const data = [
            ...
            })),
            {
                'Movimentação': 'Total:',
                'Data': '',
                'Entrada Total': totalEntrada?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }),
    ...
    const handleFiltroRelatorioMensal = async (data) => {
        ...
        showLoading(true);
        try {
            var response = await api_POST("RelatorioMensal/RelatorioMovimentacao", { dataInicial: moment(dataInicial).format("YYYY-MM-DD"), dataFinal: moment(dataFinal).format("YYYY-MM-DD") });
            const { data } = response;
            setListRelatorioMensal(data);
