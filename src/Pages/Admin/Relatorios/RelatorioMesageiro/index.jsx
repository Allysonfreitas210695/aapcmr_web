...
import { FaArrowLeft, FaFileCsv, FaFilter } from 'react-icons/fa';
...
export default function RelatorioMesageiro() {
    ...
    const [listRelatorioMensageiro, setListRelatorioMensageiro] = useState([]);

    const loadRelatorioMesageiro = async () => {
        showLoading(true);
        try {
            let response = await api_POST("RelatorioMensal/RelatorioDoacaoMessageiro", { dataInicial: null, dataFinal: null });
            const { data } = response;
    ...
    useEffect(() => {
        loadRelatorioMesageiro();
    }, []);

    const exportToExcel = async () => {
        const { totalValorDoacao } = listRelatorioMensageiro.reduce((acc, curr) => { acc.totalValorDoacao += curr.valorDoacao || 0; return acc;  },{ totalValorDoacao: 0 });

        const data = [
            ...listRelatorioMensageiro.map(item => ({
        ...
        XLSX.utils.book_append_sheet(wb, ws, "RelatorioDoacaoMesageiro");
        ...
        link.href = url;
        link.setAttribute('download', 'RelatorioDoacaoMesageiro.xlsx');
    ...
    const handleFiltroRelatorioMesageiro = async (data) => {
        ...
        showLoading(true);
        try {
            var response = await api_POST("RelatorioMensal/RelatorioDoacaoMessageiro", { dataInicial: moment(dataInicial).format("YYYY-MM-DD"), dataFinal: moment(dataFinal).format("YYYY-MM-DD") });
            const { data } = response;
            setListRelatorioMensageiro(data);
