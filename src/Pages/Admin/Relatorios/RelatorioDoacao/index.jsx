...
export default function RelatorioDoacao() {
  ...
  const loadRelatorioDoacao = async () => {
    showLoading(true);
    try {
      let response = await api_POST('RelatorioMensal/RelatorioDoacoes', {
        dataInicial: null,
        dataFinal: null
      });
      const { data } = response;
  ...
  const exportToExcel = async () => {
    const { totalValorDoacao } = listRelatorioDoacao.reduce(
      (acc, curr) => {
        acc.totalValorDoacao += curr.valorDoacao || 0;
        return acc;
      },
      { totalValorDoacao: 0 }
    );
    ...
    const data = [
      ...listRelatorioDoacao.map((item) => ({
        ...
        'Valor de Doação': item.valorDoacao?.toLocaleString('pt-BR', {
          style: 'currency',
          currency: 'BRL'
        }),
      ...
      })),
      {
        ...
        Telefone: '',
        'Valor de Doação': totalValorDoacao?.toLocaleString('pt-BR', {
          style: 'currency',
          currency: 'BRL'
        }),
        'Data da Doação': '',
  ...
  const handleFiltroRelatorioMensal = async (data) => {
    ...
    showLoading(true);
    try {
      var response = await api_POST('RelatorioMensal/RelatorioDoacoes', {
