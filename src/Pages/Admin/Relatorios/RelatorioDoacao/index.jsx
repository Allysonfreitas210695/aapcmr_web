...
export default function RelatorioDoacao() {
    ...
    const [listRelatorioDoacao, setListRelatorioDoacao] = useState([]);

    const loadRelatorioDoacao = async () => {
        showLoading(true);
        try {
            let response = await api_POST("RelatorioMensal/RelatorioDoacoes", { dataInicial: null, dataFinal: null });
            const { data } = response;
            setListRelatorioDoacao(data);
        } catch (error) {
            ShowMessage({
                title: 'Error',
    ...
    const exportToExcel = async () => {
        const { totalValorDoacao } = listRelatorioDoacao.reduce((acc, curr) => { acc.totalValorDoacao += curr.valorDoacao || 0; return acc; }, { totalValorDoacao: 0 });

        const data = [
            ...listRelatorioDoacao.map(item => ({
    ...
    const handleFiltroRelatorioMensal = async (data) => {
        ...
        showLoading(true);
        try {
            var response = await api_POST("RelatorioMensal/RelatorioDoacoes", { dataInicial: moment(dataInicial).format("YYYY-MM-DD"), dataFinal: moment(dataFinal).format("YYYY-MM-DD") });
            const { data } = response;
            setListRelatorioDoacao(data);
            ...
            {!loding &&
                <>
                    ...
                    <div className='bg-white p-3 rounded shadow'>
                        ...
                        <Col lg={12} md={12}>
                            <TableCustom
                                title='Lista de Depósitos por Doação'
                                columns={[
