...
export default function TrocaSenha() {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            showLoading(true);
            let response = await api_POST(`Usuario/TrocaSenha/${session.id}/${data.senhaAntiga}/${data.senhaNova}`);
            ShowMessage({
                title: 'Sucesso',
            ...
            <Row>
                ...
                <Col sm={12} lg={3}>
                    <ControlledInput
                        control={control}
                        name='senhaAntiga'
                        label={<><span className='text-danger'>*</span> Senha Antiga</>}
                        type='password'
                        rules={{
                            required: true,
                            minLength: {
                ...
                <Col sm={12} lg={3}>
                    <ControlledInput
                        control={control}
                        name='senhaNova'
                        label={<><span className='text-danger'>*</span> Senha Nova</>}
                        type='password'
                        rules={{
                            required: true,
                            minLength: {
                ...
                <Col sm={12} lg={3}>
                    <ControlledInput
                        ...
                        label={<><span className='text-danger'>*</span> Confirme Senha</>}
                        type='password'
                        rules={{
                            required: true,
                            minLength: {
